...
class AsyncFileWriter:
    def __init__(self, platform: str, crawler_type: str):
        ...
        self.keywords = config.KEYWORDS.replace(",", "_")

    def _get_file_path(self, file_type: str, item_type: str) -> str:
        base_path = f"data/{self.platform}/{file_type}"
        pathlib.Path(base_path).mkdir(parents=True, exist_ok=True)
        file_name = f"{self.crawler_type}_{item_type}_{utils.get_current_date()}_{self.keywords}.{file_type}"
    ...
    async def generate_wordcloud_from_comments(self):
        ...
        try:
            ...
            if not os.path.exists(comments_file_path) or os.path.getsize(comments_file_path) == 0:
                utils.logger.info(
                    f"[AsyncFileWriter.generate_wordcloud_from_comments] No comments file found at {comments_file_path}")
                return
