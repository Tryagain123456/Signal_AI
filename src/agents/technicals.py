...
# 初始化 logger
logger = setup_logger('technical_analysis_tool')


##### technical_analysis_tool #####
# "technical_analyst", "技术分析师，提供基于价格走势、指标和技术模式的交易信号")
def technical_analysis_tool(state: AgentState):
    """
    ...
    show_workflow_status("technical_analysis_tool")
    ...
    # Create the technical_analysis_tool message
    ...
    if show_reasoning:
        show_agent_reasoning(analysis_report, "technical_analysis_tool")
        # 保存推理信息到state的metadata供API使用
        state["metadata"]["agent_reasoning"] = analysis_report

    show_workflow_status("technical_analysis_tool", "completed")
